...
int readin(int prompt) {
  ...
  if(this_node==0){
    ...
    /************************************************ NEW WP **********************************************************/
     /*------------------------------------------------------------*/
    /* Tetraquark correlators                                          */
    /*------------------------------------------------------------*/
    
    /* Number of quark quads */
    IF_OK status += get_i(stdin,prompt,"number_of_tetraquarks", &param.num_quad );
    if( param.num_quad>MAX_QUAD ){
      printf("num_quad = %d must be <= %d!\n", param.num_quad, MAX_QUAD);
      status++;
    }
    
    IF_OK for(iquad = 0; iquad < param.num_quad; iquad++){
      char request_buf[MAX_SPECTRUM_REQUEST];
      
      /* Which quarks in the pair? */
      
      IF_OK status += get_vi(stdin, prompt, "quad", param.qkquad[iquad], 4);
      
      IF_OK {
	int j;
	for(j = 0; j < 4; j++){
	  if(param.qkquad[iquad][j] < 0 || param.qkquad[iquad][j] >= param.num_qk){
	    printf("Quark index %d must be in [0,%d]\n",
		   param.qkquad[iquad][j],param.num_qk-1);
	    status++;
	  }
	}
      }
      ...
      IF_OK {
	/* Mesons. The only choice for now. */
	if(strstr(request_buf,",tetraquark,") != NULL)
	  param.do_tetraquark_spect[iquad] = 1;
	else param.do_tetraquark_spect[iquad] = 0;
	
	/* We need some correlator specification */
	if(!param.do_tetraquark_spect[iquad]){
	  printf("Unrecognized spectrum request\n");
	  status++;
	}
      }
      ...
      IF_OK status += ask_corr_file( stdin, prompt, &param.saveflag_t[iquad],
				     param.savefile_t[iquad]);
      /* Correlator values are printed with t relative to the t_offset_m */
      /* FT phases are computed with x,y,z relative to r_offset_m */
      IF_OK {
	int r[4];
	status += get_vi(stdin,prompt, "r_offset", r, 4);
	param.r_offset_t[iquad][0] = r[0];
	param.r_offset_t[iquad][1] = r[1];
	param.r_offset_t[iquad][2] = r[2];
	param.r_offset_t[iquad][3] = r[3];
      }
      ...
      /* Number of correlators */
      IF_OK status += get_i(stdin,prompt,"number_of_correlators", 
			    &param.num_corr_t[iquad] );
      IF_OK {
	if( param.num_corr_t[iquad]>MAX_CORR ){
	  printf("num_corr = %d must be <= %d!\n", param.num_corr_t[iquad], 
		 MAX_CORR);
	  status++;
	}
	 ...
	 correlator  A1_P5 p200 -i * 1 2 0 0 E E E */
      ...
      param.num_corr_report_tetraquark[iquad] = 0;
      IF_OK for(i = 0; i < param.num_corr_t[iquad]; i++){
	int ok,m;
	char tetraquark_label_in[MAX_TETRAQUARK_LABEL], mom_label_in_tetraquark[MAX_MOM_LABEL],
	  spin_taste_string_tetraquark[8], phase_lab_tetraquark[4], 
	  factor_op_tetraquark[2], parity_x_in_tetraquark[3], parity_y_in_tetraquark[3], parity_z_in_tetraquark[3];
	double factor;
	
	IF_OK status += get_sn(stdin, prompt, "correlator", tetraquark_label_in);
	
	/* Read the momentum label next */
	IF_OK {
	  ok = scanf("%s", mom_label_in_tetraquark);
	  if(ok != 1){
	    printf("Error reading tetraquark momentum label\n");
	    status++;
	  }
	}
	IF_OK printf(" %s", mom_label_in_tetraquark);
	
	/* Add to hash table */
	m = hash_corr_label(param.tetraquark_label[iquad], param.mom_label_tetraquark[iquad],
			    tetraquark_label_in, mom_label_in_tetraquark,
			    &param.num_corr_report_tetraquark[iquad]);
	param.corr_index_tetraquark[iquad][i] = m;
	if(m < 0)status++;
	
	/* phase, op, factor, and spin-taste label */
	IF_OK {
	  ok = scanf("%s %s %lf %s",phase_lab_tetraquark,factor_op_tetraquark,&factor,
		     spin_taste_string_tetraquark);
	  if(ok != 4){
	    printf("\nError reading tetraquark phase, factor, and spin-taste\n");
	    status++;
	  }
	  else {
	    printf(" %3s %1s %g %6s",phase_lab_tetraquark,factor_op_tetraquark,factor,
		   spin_taste_string_tetraquark);
	  }
	}
	...
	/* decode phase for correlator */
	IF_OK {
	  param.tetraquark_phase[iquad][i] = decode_phase(phase_lab_tetraquark);
	  if(param.tetraquark_phase[iquad][i] == -1){
	    printf("\n%s is not a valid tetraquark phase label\n",phase_lab_tetraquark);
	    status ++;
	  }
	}
	...
	/* Permitted syntax is /ddd or *ddd where ddd is a real number */
	IF_OK {
	  param.tetraquark_factor[iquad][i] = decode_factor(factor_op_tetraquark,factor);
	  if(param.tetraquark_factor[iquad][i] == 0 ){
	    printf("\n: Decoding tetraquark factor %s %g results in zero.\n",
		   factor_op_tetraquark,factor);
	    status++;
	  }
	}
	...
	/* decode spin-taste sink label */
	IF_OK {
	  param.spin_taste_snk_tetraquark[iquad][i] = spin_taste_index_tetraquark(spin_taste_string_tetraquark);
	  if(param.spin_taste_snk_tetraquark[iquad][i] < 0 ){
	    printf("\n: Unrecognized tetraquark spin-taste label %s.\n",
		   spin_taste_string);
	    status++;
	  }
	}
	...
	/* momentum indices */
	IF_OK {
	  if(scanf("%d %d %d",&param.corr_mom_tetraquark[iquad][i][0],
		   &param.corr_mom_tetraquark[iquad][i][1],
		   &param.corr_mom_tetraquark[iquad][i][2]) != 3){
	    printf("\nFormat error on tetraquark momentum line\n");
	    status++;
	  }
	  
	  IF_OK printf(" %2d %2d %2d",param.corr_mom_tetraquark[iquad][i][0],
		       param.corr_mom_tetraquark[iquad][i][1],
		       param.corr_mom_tetraquark[iqyad][i][2]);
	}
	
	/* parity for each momentum component */
	
	IF_OK {
	  ok = scanf("%s %s %s", parity_x_in_tetraquark, parity_y_in_tetraquark, parity_z_in_tetraquark);
	  if(ok != 3){
	    printf("\nError reading tetraquark parity label\n");
	    status++;
	  }
	  IF_OK printf(" %2s %2s %2s\n", parity_x_in_tetraquark, parity_y_in_tetraquark, parity_z_in_tetraquark);
	}
	
	/* decode parity labels */
	IF_OK {
	  param.corr_parity_tetraquark[iquad][i][0] = decode_parity(parity_x_in_tetraquark);
	  param.corr_parity_tetraquark[iquad][i][1] = decode_parity(parity_y_in_tetraquark);
	  param.corr_parity_tetraquark[iquad][i][2] = decode_parity(parity_z_in_tetraquark);
	  if(param.corr_parity_tetraquark[iquad][i][0] == 0xf)status++;
	  if(param.corr_parity_tetraquark[iquad][i][1] == 0xf)status++;
	  if(param.corr_parity_tetraquark[iquad][i][2] == 0xf)status++;
	}
      } /* correlators for this pair */
    } /* pairs */
