...
{
  int prompt;
  int i, j, k, iq0, iq1, iq2, iq3;   /* Added iq3 WP */
#ifdef KS_LEAN
  int oldiq0, oldiq1, oldiq2, oldiq3, oldip0;  /* Added oldiq3 WP */
  int quark_nc[MAX_QK];
#endif
  int naik_index, naik_index0, naik_index1, naik_index2, naik_index3;  /* 2 and 3 added by WP */
  double mass;
  ...
  while( readin(prompt) == 0){
    ...
    for(i = 0; i < param.num_triplet; i++){
      ...
      /* Index for the quarks making up this baryon */
    ...
    /************** TETRAQURAK WP *********************************************************************************/
    /****************************************************************/
    /* Compute the tetraquark propagators */
    
    STARTTIME;
    for(i = 0; i < param.num_quad; i++){
      
      /* Index for the quarks making up this tetraquark */
      iq0 = param.qkquad[i][0];
      iq1 = param.qkquad[i][1];
      iq2 = param.qkquad[i][2];
      iq3 = param.qkquad[i][3];
      /* Naik indices for the quarks */
      naik_index0 = param.naik_index[iq0];
      naik_index1 = param.naik_index[iq1];
      naik_index2 = param.naik_index[iq2];
      naik_index3 = param.naik_index[iq3];
      node0_printf("Tetraquarks for quarks %d, %d, %d, and %d\n",iq0,iq1,iq2,iq3);
      /* why are there no Naik indicies in the baryon like in the meson case? Problably never used HISQ */
#ifdef KS_LEAN
      /* Restore quarks from file and free old memory */
      /* We try to reuse props that are already in memory, so we don't
         destroy them immediately, but wait to see if we need
         them again for the next pair. */
      if(i > 0 && oldiq0 != iq0 && oldiq0 != iq1 && oldiq0 != iq2 && oldiq0 != iq3)
	if(param.saveflag_q[oldiq0] != FORGET){
	  destroy_ksp_field(quark[oldiq0]); quark[oldiq0] = NULL;
	  node0_printf("destroy quark[%d]\n",oldiq0);
	}
      
      if(i > 0 && oldiq1 != iq0 && oldiq1 != iq1 && oldiq1 != iq2 && oldiq1 != iq3)
	if(param.saveflag_q[oldiq1] != FORGET){
	  destroy_ksp_field(quark[oldiq1]); quark[oldiq1] = NULL;
	  node0_printf("destroy quark[%d]\n",oldiq1);
	}
      
      if(i > 0 && oldiq2 != iq0 && oldiq2 != iq1 && oldiq2 != iq2 && oldiq2 != iq3)
	if(param.saveflag_q[oldiq2] != FORGET){
	  destroy_ksp_field(quark[oldiq2]); quark[oldiq2] = NULL;
	  node0_printf("destroy quark[%d]\n",oldiq2);
	}

      if(i > 0 && oldiq3 != iq0 && oldiq3 != iq1 && oldiq3 != iq2 && oldiq3 != iq3)
	if(param.saveflag_q[oldiq3] != FORGET){
	  destroy_ksp_field(quark[oldiq3]); quark[oldiq3] = NULL;
	  node0_printf("destroy quark[%d]\n",oldiq3);
	}
	...
	quark[iq2] = 
      ...
      if(quark[iq3] == NULL)
	quark[iq3] = 
	  reread_ksprop_to_ksp_field(param.saveflag_q[iq3], 
				     param.savefile_q[iq3],
				     quark_nc[iq3]);
#endif
      
      /* Tie together to generate tetraquark spectrum edited WP*/
      spectrum_ks_tetraquark(quark[iq0], naik_index0, quark[iq1], naik_index1, quark[iq2], naik_index2, quark[iq3], naik_index3, i);
    ...
    if(quark[oldiq3] != NULL)
      if(param.saveflag_q[oldiq3] != FORGET){
	destroy_ksp_field(quark[oldiq3]); quark[oldiq3] = NULL;
	node0_printf("destroy quark[%d]\n",oldiq3);
      }
#endif
    ENDTIME("tie tetraquark correlators");
    
    /* End of bit edited for tetraquarks */
