...
#define MAX_QUAD 16 /* Arbritarily added by WP */
...
#define MAX_TETRAQUARK_LABEL 32
...
typedef struct {
  ...
  int num_quad;                      /* Number of tetraquuarks WP*/
  int qkquad[MAX_QUAD][4];           /* Indices of quarks in a tetraquark WP*/ 
  ...
  int do_tetraquark_spect[MAX_QUAD];  /* New WP */
  ...
  int saveflag_t[MAX_QUAD];            /* New WP*/
  char savefile_t[MAX_QUAD][MAXFILENAME]; /* New WP*/
  ...
  int r_offset_t[MAX_QUAD][4];
  int num_corr_m[MAX_PAIR];                  /* Number of correlators for a meson */
  int num_corr_b[MAX_TRIPLET];             /* Number of correlators for a baryon */
  int num_corr_t[MAX_QUAD];                /* Number of correlators for a tetraquark WP */
  int num_corr_report[MAX_PAIR];           /* Number of correlators to report for a meson */
  int num_corr_report_tetraquark[MAX_QUAD];           /* Number of correlators to report for a tetraquark */
  char meson_label[MAX_PAIR][MAX_CORR][MAX_MESON_LABEL];
  char tetraquark_label[MAX_QUAD][MAX_CORR][MAX_TETRAQUARK_LABEL]; /* New WP */
  char mom_label[MAX_PAIR][MAX_CORR][MAX_MOM_LABEL];
  char mom_label_tetraquark[MAX_QUAD][MAX_CORR][MAX_MOM_LABEL]; /* NEW WP*/
  int corr_index[MAX_PAIR][MAX_CORR];
  int corr_index_tetraquark[MAX_QUAD][MAX_CORR];   /*  Need this for tetraquarks? WP*/  
  int spin_taste_snk[MAX_PAIR][MAX_CORR];
  int spin_taste_snk_tetraquark[MAX_QUAD][MAX_CORR];      /* tetraquark */
  int baryon_type_snk[MAX_TRIPLET][MAX_CORR];
  int meson_phase[MAX_PAIR][MAX_CORR];
  Real meson_factor[MAX_PAIR][MAX_CORR];
  int tetraquark_phase[MAX_QUAD][MAX_CORR]; /* new WP */
  Real tetraquark_factor[MAX_QUAD][MAX_CORR]; /* new WP */
  ...
  int corr_mom_tetraquark[MAX_QUAD][MAX_CORR][3];    /* New for tetraquark WP */
  char corr_parity_tetraquark[MAX_QUAD][MAX_CORR][3]; /* New for tetraquark WP */
