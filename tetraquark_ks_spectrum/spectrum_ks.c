...
static complex **ptet_prop = NULL; /* New WP */

/* Table of unique sink spin-taste indices */
static int *spin_taste_table, num_spin_taste_indices; 
static int *spin_taste_table_tetraquark, num_spin_taste_indices_tetraquark;
/* Table of unique momentum/parity combinations */
static int **momentum, num_mom_parities;
static int **momentum_tetraquark, num_mom_parities_tetraquark;  /* NEW WP */
char **parity;
char **parity_tetraquark;  /* NEW WP */
...
static int *spin_taste_indices_tetraquark, *mom_parity_index_tetraquark; /* NEW WP */
static int **corr_table_tetraquark, *num_corr_mom_parity_tetraquark;   /* NEW WP */
...
static void 
create_mom_parity_tetraquark(int ***mom, char ***par, int **mp_index, int *nmp, int quad){
  int c;
  int num_corr = param.num_corr_t[quad];
  char myname[] = "create_mom_parity_tetraquark";

  *mom = (int **)malloc(num_corr*sizeof(int *));
  if(*mom == NULL){
    printf("%s(%d): no room for tetraquark mom table\n",myname,this_node);
    terminate(1);
  }
  
  *par = (char **)malloc(num_corr*sizeof(int *));
  if(*par == NULL){
    printf("%s(%d): no room for tetraquark parity table\n",myname,this_node);
    terminate(1);
  }

  *mp_index = (int *)malloc(num_corr*sizeof(int));
  if(*mp_index == NULL){
    printf("%s(%d): no room for tetraquark mp_index\n",myname,this_node);
    terminate(1);
  }
  ...
  for(c = 0; c < num_corr; c++){
    (*mp_index)[c] = 
      hash_meson_mom_parity(*mom, *par,
			    param.corr_mom_tetraquark[quad][c], 
			    param.corr_parity_tetraquark[quad][c],
			    nmp);    /* Does not need editing but pass different htings WP*/
  }
}
...
static void 
destroy_mom_parity_tetraquark(int ***mom, char ***par, int **mp_index, int nmp, int quad){
...
static void 
create_spin_taste_index_tetraquark(int **st, int **st_index, int *n, int quad)
{
  char myname[] = "create_spin_tate_index_tetraquark";
  int c;
  int num_corr = param.num_corr_t[quad];

  *st = (int *)malloc(num_corr*sizeof(int));
  if(*st == NULL){
    printf("%s(%d): no room for tetraquark spin-taste table\n", myname, this_node);
    terminate(1);
  }
  
  *st_index = (int *)malloc(num_corr*sizeof(int));
  if(*st_index == NULL){
    printf("%s(%d): no room for tetraquark spin-taste index table\n", myname, this_node);
    terminate(1);
  }
  ...
  for(c = 0; c < num_corr; c++){
    (*st_index)[c] = 
      hash_spin_taste_index(*st, param.spin_taste_snk_tetraquark[quad][c], n);  /* does not need modifying but pass different param WP */
  }
}
/*--------------------------------------------------------------------*/
...
/************************ NEW WP **************************************/
static int *
create_num_corr_mom_parity_tetraquark(int *st_index, int n, int quad)
{
  char myname[] = "create_num_corr_mom_parity_tetraquark";
  int num_corr = param.num_corr_t[quad];
  int c, g;
  int *n_cmp;
  ...
  n_cmp = (int *)malloc(n*sizeof(int));
  if(n_cmp == NULL){
    printf("%s(%d): no room for tetraquark num corr table\n",myname,this_node);
    terminate(1);
  }
...
static void
fill_corr_table_tetraquark(int *st, int *st_index, int **c_table, 
		int *n_cmp, int n, int *mp_index, int quad)
{
  char myname[] = "fill_corr_table_tetraquark";
  int c, g, k;
  int num_corr = param.num_corr_t[quad];
  int *k_corr_mom_parity;
  ...
  k_corr_mom_parity = (int *)malloc(n*sizeof(int));
  if(k_corr_mom_parity == NULL){
    printf("%s(%d): no room for tetraquark counter\n",myname,this_node);
    terminate(1);
  }
  ...
  for(g = 0; g < n; g++){
    c_table[g] = (int *)malloc(sizeof(int)*n_cmp[g]);
    if(c_table[g] == NULL){
      printf("%s(%d): no room for tetraquark corr table\n",myname, this_node);
       terminate(1);
     }
  }
  ...
  free(k_corr_mom_parity);  /* can't find this but assume doesn't need editing WP */
}
/*--------------------------------------------------------------------*/
...
static void
create_corr_table_tetraquark(int quad)
{

  char myname[] = "create_corr_table_tetraquark";  /* modified WP */

  num_spin_taste_indices_tetraquark = 0;
  num_mom_parities_tetraquark = 0;

  create_spin_taste_index_tetraquarks(&spin_taste_table_tetraquark, &spin_taste_indices_tetraquark,
				      &num_spin_taste_indices_tetraquark, quad);   /* new function WP */


  create_mom_parity_tetraquark(&momentum_tetraquark, &parity_tetraquark, &mom_parity_index_tetraquark, 
			       &num_mom_parities_tetraquark, quad);     /* new function WP */

  num_corr_mom_parity_tetraquark = 
    create_num_corr_mom_parity_tetraquark(spin_taste_indices_tetraquark, num_spin_taste_indices_tetraquark, 
					  quad);   /* new function WP */

  /* Create the correlator table */
  
  corr_table_tetraquark = (int **)malloc(num_spin_taste_indices_tetraquark*sizeof(int *)); /* NEW WP */
  if(corr_table == NULL){
    printf("%s(%d): no room for tetraquark corr table\n",myname, this_node);
    terminate(1);
  }
  ...
  fill_corr_table_tetraquark(spin_taste_table_tetraquark, spin_taste_indices_tetraquark, corr_table_tetraquark,
		  num_corr_mom_parity_tetraquark, num_spin_taste_indices_tetraquark, 
		  mom_parity_index_tetraquark, quad);
...
/*--------------------------------------------------------------------*/
static void
destroy_corr_table_tetraquark(int quad)
{
  int g;

  destroy_spin_taste_index(&spin_taste_table_tetraquark, &spin_taste_indices_tetraquark);

  destroy_mom_parity_tetraquark(&momentum_tetraquark, &parity_tetraquark, &mom_parity_index_tetraquark,
				num_mom_parities_tetraquark, quad); /* NEW WP */

  destroy_num_corr_mom_parity(&num_corr_mom_parity_tetraquark); 

  if(corr_table != NULL){
    for(g = 0; g < num_spin_taste_indices_tetraquark; g++){
      if(corr_table_tetraquark[g] != NULL)
	free(corr_table_tetraquark[g]);
    }
    free(corr_table_tetraquark);
    corr_table_tetraquark = NULL;
  }
}
...
/*--------------------------------------------------------------------*/
/*****************************NEW FOR TETRAQUARKS **************************/
/* The num_corr_occur_tetraquark array counts the number of contributions to
   a given tetraquark correlator for normalization purposes    */

static int *
create_num_corr_occur_tetraquark(int quad){
  int c,m;
  int ncr = param.num_corr_report_tetraquark[quad];
  int num_corr = param.num_corr_t[quad];
  int *nco;
  char myname[] = "create_num_corr_occur_tetraquark";

  nco = (int *)malloc(sizeof(int)*ncr);
  if(nco == NULL){
    printf("%s(%d) no room for num_corr_occur_tetraquark\n", myname, this_node);
    terminate(1);
  }
  ...
  for(c = 0; c < num_corr; c++){
    m = param.corr_index_tetraquark[quad][c];
    nco[m]++;
  }
...
/*--------------------------------------------------------------------*/
static void 
spectrum_ks_tetraquark_init(int quad){
  
  int num_corr = 
    param.num_corr_report_tetraquark[quad]; /* number of tetraquark correlators to store */

  if(param.do_tetraquark_spect[quad]) /* New param WP */
    ptet_prop = create_hadron_prop(num_corr, nt);  /* changed name but func does not need editing WP */

  /* Set up count of contributions to the correlators */
  num_corr_occur_tetraquark = create_num_corr_occur_tetraquark(quad);   /* New function WP */
  
  /* Set up table of requested correlators */
  create_corr_table_tetraquark(quad);    /* New function WP */   
...
/*--------------------------------------------------------------------*/
/*********************************NEW WP ******************************/
static void accum_gen_tetraquark(complex **mp, su3_vector *qp0, int naik_index0,
			    su3_vector *qp1, int naik_index1,
			    su3_vector *qp2, int naik_index2,
			    su3_vector *qp3, int naik_index3, int quad){

  imp_ferm_links_t **fn = get_fm_links(fn_links);

  ks_tetraquark_cont_mom(mp, qp0, qp1, qp2, qp3, 
		    num_mom_parities_tetraquark, momentum_tetraquark, parity_tetraquark,
		    num_spin_taste_indices_tetraquark, num_corr_mom_parity_tetraquark, corr_table_tetraquark, 
		    mom_parity_index_tetraquark, fn[naik_index0], fn[naik_index1], fn[naik_index2], fn[naik_index3], 
		    param.spin_taste_snk_tetraquark[quad], 
		    param.tetraquark_phase[quad], param.tetraquark_factor[quad],
			 param.corr_index_tetraquark[quad], &param.r_offset_t[quad][0]);  /* NEW WP in generic_ks/ks_meson_mom.c edit later*/ 
}
...
/************************************** NEW WP *************************************/
/*--------------------------------------------------------------------*/
static void spectrum_ks_offdiag_gen_tetraquark(ks_prop_field *qp0, 
					  int naik_index0,
					  ks_prop_field *qp1, 
					  int naik_index1,
					  ks_prop_field *qp2, 
  ...
  su3_vector *qps0, *qps1, *qps2, *qps3;
  int color;
  double dtime = start_timing();

  if(qp0->nc != qp1->nc){
    printf("spectrum_ks_offdiag_gen_meson(%d): mismatched number of colors in pair %d: %d != %d\n", this_node, quad, qp0->nc, qp1->nc);
    terminate(1);
  }
  if(qp0->nc != qp2->nc){
    printf("spectrum_ks_offdiag_gen_tetraquark(%d): mismatched number of colors in pair %d: %d != %d\n", this_node, quad, qp0->nc, qp2->nc);
    terminate(1);
  }

  if(qp0->nc != qp3->nc){
    printf("spectrum_ks_offdiag_gen_tetraquark(%d): mismatched number of colors in pair %d: %d != %d\n", this_node, quad, qp0->nc, qp3->nc);
    terminate(1);
  }
  if(qp1->nc != qp2->nc){
    printf("spectrum_ks_offdiag_gen_tetraquark(%d): mismatched number of colors in pair %d: %d != %d\n", this_node, quad, qp1->nc, qp2->nc);
    terminate(1);
  }

  if(qp1->nc != qp3->nc){
    printf("spectrum_ks_offdiag_gen_tetraquark(%d): mismatched number of colors in pair %d: %d != %d\n", this_node, quad, qp1->nc, qp3->nc);
    terminate(1);
  }
  if(qp2->nc != qp3->nc){
    printf("spectrum_ks_offdiag_gen_tetraquark(%d): mismatched number of colors in pair %d: %d != %d\n", this_node, quad, qp2->nc, qp3->nc);
    terminate(1);
  }
  ...
  for(color = 0; color < qp0->nc; color++){
    ...
    qps3 = qp3->v[color];
    /* Point meson propagator */
    accum_gen_tetraquark(mp, qps0, naik_index0, qps1, naik_index1, qps2, naik_index2, qps3, naik_index3, quad);    /*New fucntion WP*/
  }
  print_timing(dtime, "offdiag tetraquarks");
}
...
/************************** NEW WP *****************************************/
/*--------------------------------------------------------------------*/
static void spectrum_ks_offdiag_tetraquark(ks_prop_field *qp0, 
				      int naik_index0,
				      ks_prop_field *qp1,
				      int naik_index1,
				      ks_prop_field *qp2,
  ...
  /* Point meson */
  spectrum_ks_offdiag_gen_tetraquark(qp0, naik_index0, qp1, naik_index1, qp2, naik_index2, qp3, naik_index3, ptet_prop, quad);  /* NEW WP */
}
...
/*--------------------------------------------------------------------*/
static FILE* open_fnal_tetraquark_file(int quad){
  int iq0 = param.qkquad[quad][0];
  int iq1 = param.qkquad[quad][1];
  int iq2 = param.qkquad[quad][2];
  int iq3 = param.qkquad[quad][3];
  int ih0[MAX_HISTORY], ih1[MAX_HISTORY], ih2[MAX_HISTORY], ih3[MAX_HISTORY];
  int nh0, nh1, nh2, nh3;
  int k;
  int ip0 = get_ancestors(ih0, &nh0, iq0);
  int ip1 = get_ancestors(ih1, &nh1, iq1);
  int ip2 = get_ancestors(ih2, &nh2, iq2);
  int ip3 = get_ancestors(ih3, &nh3, iq3);
  int is0 = param.set[ip0];
  int is1 = param.set[ip1];
  int is2 = param.set[ip2];
  int is3 = param.set[ip3];
  FILE *fp;

  /* Only node 0 writes, and only if we want the file. */
  if(this_node != 0 || param.saveflag_t[quad] == FORGET )
    return NULL;

  /* Always append */
  fp = fopen(param.savefile_t[quad],"a");
  if(fp == NULL){
    printf("open_fnal_tetraquark_file: ERROR. Can't open %s\n",
	   param.savefile_t[quad]);
    return NULL;
  }
  fprintf(fp,"---\n");
  fprintf(fp,"JobID:                        %s\n",param.job_id);
  ...
  fprintf(fp,"antiquark_type:               staggered\n");
  /*********************************************** Can edit the rest later as not relevant  WP  **********************************/
//  {
//    quark_source_sink_op* qs_op = param.src_qs[is0].op;
  print_source_info(fp, "antiquark_source", &param.src_qs[is0]);
  fprintf(fp,"antiquark_source_label:       %s\n",param.src_qs[is0].label);  
...
/*********** NEW WP ***************************************************/
/*--------------------------------------------------------------------*/
static int lookup_corr_index_tetraquark(int quad, int m){
  int i;

  /* Search corr_index table for first occurrence of correlator index m */
  for(i = 0; i < param.num_corr_t[quad]; i++){
    if(param.corr_index_tetraquark[quad][i] == m)
      return i;
  }
  printf("lookup_corr_index_tetraquark: Can't locate correlator %d for quad %d\n",
	 m,quad);
  terminate(1);
  return -1;
}
...
/************************************* NEW WP ************************************/ 
/*--------------------------------------------------------------------*/
static void print_start_fnal_tetraquark_prop(FILE *fp, int quad, int m)
{
  int iq0 = param.qkquad[quad][0];
  int iq1 = param.qkquad[quad][1];
  int iq2 = param.qkquad[quad][2];
  ...
  int is2 = param.set[ip2];
  int is3 = param.set[ip3];
  int i   = lookup_corr_index_tetraquark(quad,m); /*WP*/

  if(this_node != 0 || param.saveflag_t[quad] == FORGET)return;

  fprintf(fp,"---\n");
  fprintf(fp,"correlator:                   %s\n",param.tetraquark_label[quad][m]);
  fprintf(fp,"momentum:                     %s\n",param.mom_label_tetraquark[quad][m]);
  fprintf(fp,"spin_taste_sink:              %s\n",
	  spin_taste_label(param.spin_taste_snk_tetraquark[quad][i]));

  /* Print correlator key encoding metadata */
  fprintf(fp,"correlator_key:               %s", param.tetraquark_label[quad][m]);
  ...
  if(strlen(param.src_qs[is3].label)>0)
    fprintf(fp,"_%s", param.src_qs[is3].label);
  /* Sink labels */
  if(strlen(param.snk_qs_op[iq0].label)>0)
    fprintf(fp,"_%s",param.snk_qs_op[iq0].label);
  ...
  if(strlen(param.snk_qs_op[iq3].label)>0)
    fprintf(fp,"_%s",param.snk_qs_op[iq3].label);
  /* Mass labels */
  fprintf(fp,"_m%s", param.mass_label[ip0]);
#if U1_FIELD
  fprintf(fp,"_q%s", param.charge_label[is0]);
...
#endif
  fprintf(fp,"_m%s", param.mass_label[ip3]);
#if U1_FIELD
  fprintf(fp,"_q%s", param.charge_label[is3]);
#endif
  fprintf(fp, "_%s\n", param.mom_label_tetraquark[quad][m]);
...
/*--------------------------------------------------------------------*/
static void 
print_start_tetraquark_prop(int pair, int m){
  if(param.saveflag_t[quad] != FORGET)return;
  int iq0 = param.qkquad[quad][0];
  int iq1 = param.qkquad[quad][1];
  int iq2 = param.qkquad[quad][2];
  int iq3 = param.qkquad[quad][3];
  ...
  printf("%g ", param.ksp[ip0].mass);
  printf("%g ", param.ksp[ip1].mass);
  printf("%g ", param.ksp[ip2].mass);
  printf("%g ", param.ksp[ip3].mass);
  printf("\n");
#ifdef U1_FIELD
  printf("CHARGES: ");
  printf("%g ", param.charge[is0]);
  printf("%g ", param.charge[is1]);
  printf("%g ", param.charge[is2]);
  printf("%g ", param.charge[is3]);
  printf("\n");
#endif
  printf("SOURCE: %s %s %s %s\n",param.src_qs[is0].descrp,
	 param.src_qs[is1].descrp, param.src_qs[is2].descrp , param.src_qs[is3].descrp);
  printf("SINKOPS: %s %s %s %s\n", param.snk_qs_op[iq0].descrp, 
	 param.snk_qs_op[iq1].descrp, param.snk_qs_op[iq2].descrp, param.snk_qs_op[iq3].descrp);
  printf("SINKS: %s\n", param.tetraquark_label[quad][m]);
}
...
/************************ NEW WP ***************************************/
/*--------------------------------------------------------------------*/
static void print_tetraquark_prop(int quad, int t, complex c)
{
  if(param.saveflag_t[quad] != FORGET)return;
  node0_printf("%d %e %e\n",t,(double)c.real,(double)c.imag);
}
...
/************************* NEW WP **************************************/
/*--------------------------------------------------------------------*/
static void print_fnal_tetraquark_prop(FILE *fp, int quad, int t, complex c)
{
  if(this_node != 0 || param.saveflag_t[quad] == FORGET)return;
  fprintf(fp, "%d\t%e\t%e\n", t, (double)c.real, (double)c.imag);
}
...
/************************ NEW WP **************************************/
/*--------------------------------------------------------------------*/
static void print_end_tetraquark_prop(int quad){
  if(param.saveflag_t[quad] != FORGET)return;
  node0_printf("ENDPROP\n");
}
/*--------------------------------------------------------------------*/
...
/**************************** NEW WP **********************************/
/*--------------------------------------------------------------------*/
static void print_end_fnal_tetraquark_prop(FILE *fp, int quad){
  if(this_node != 0 || param.saveflag_t[quad] == FORGET)return;
  //  fprintf(fp, "&\n");
}
/*--------------------------------------------------------------------*/
...
/********************************** NEW WP *****************************/
/*--------------------------------------------------------------------*/
static void close_fnal_tetraquark_file(FILE *fp, int quad){
  if(this_node != 0 || param.saveflag_t[quad] == FORGET)return;
  if(fp != NULL)fclose(fp);
}
/*--------------------------------------------------------------------*/
...
/********************************** NEW WP *********************************/
/*--------------------------------------------------------------------*/
static void spectrum_ks_print_offdiag_tetraquark(int quad){
  
  //  Real space_vol;
  Real norm_fac;
  int t, tp;
  int m;
  int num_report = param.num_corr_report_tetraquark[quad];
  complex prop;
  FILE *corr_fp;
  ...
  /* Point sink */
  if(param.do_tetraquark_spect[quad]){
    corr_fp = open_fnal_tetraquark_file(quad); /* New function WP not fully edited*/
    if(this_node == 0 && corr_fp == NULL)
      param.saveflag_t[quad] = FORGET;

    /* print tetraquark propagators */
    for(m=0;m<num_report;m++) {
      norm_fac = num_corr_occur_tetraquark[m];
      
      print_start_tetraquark_prop(quad, m);  /* new function WP */
      print_start_fnal_tetraquark_prop(corr_fp, quad, m); /* New function WP */
      for(t=0; t<nt; t++){
	tp = (t + param.r_offset_t[quad][3]) % nt;
	prop = ptet_prop[m][tp];
	g_complexsum( &prop );
	CDIVREAL(prop, norm_fac, prop);
	print_tetraquark_prop(quad, t, prop); /* NEW WP */
	print_fnal_tetraquark_prop(corr_fp, quad, t, prop); /* NEW WP */
      }
      print_end_tetraquark_prop(quad);  /* NEW WP */
      print_end_fnal_tetraquark_prop(corr_fp, quad);  /* NEW WP */
    } /* mesons and momenta */
    close_fnal_tetraquark_file(corr_fp, quad);  /* NEW WP */
  }
}
/*--------------------------------------------------------------------*/
...
/*************************** NEW WP ************************************/
/*--------------------------------------------------------------------*/
void spectrum_ks_tetraquark_cleanup(int quad){
  int num_corr = param.num_corr_report_tetraquark[quad];

  if(param.do_tetraquark_spect[quad])
    destroy_hadron_prop(&ptet_prop, num_corr);

  destroy_corr_table_tetraquark(quad); /* New WP */

  destroy_num_corr_occur(&num_corr_occur); /* no need to edit*/ 
}
...
/*********************************** NEW FOR TETRAQUARK WP ***************************************************/
/*--------------------------------------------------------------------*/
void spectrum_ks_tetraquark(ks_prop_field *qp0, int naik_index0, 
		 ks_prop_field *qp1, int naik_index1, ks_prop_field *qp2, int naik_index2, ks_prop_field *qp3, int naik_index3, int quad)
{

  int *qkquad = param.qkquad[quad];
  double dtime;

  spectrum_ks_tetraquark_init(quad);   /* New function WP */

  if(qkquad[0] == qkquad[2]){   /* want to feed quarks in order \bar{h} \bar{h} l l Have only changed offdiag case  */ 
    ...
    if(param.do_tetraquark_spect[quad]){
      spectrum_ks_offdiag_tetraquark(qp0, naik_index0, qp1, naik_index1, qp2, naik_index2, qp3, naik_index3, quad); /* New function Wp*/

      dtime = start_timing();
      spectrum_ks_print_offdiag_tetraquark(quad); /* NEW WP */ 
      print_timing(dtime, "printing correlator");
    }
  }    
	    
  spectrum_ks_tetraquark_cleanup(quad);  /* NEW WP */
}
/*--------------------------------------------------------------------*/
/***********************************************END OF NEW **************************************************/
